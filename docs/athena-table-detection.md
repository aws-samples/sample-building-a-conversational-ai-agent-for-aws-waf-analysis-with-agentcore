...
2. **Search Glue Data Catalog** for an existing table that matches the S3 path and has WAF log columns (`action`, `httprequest`)
...
|----------|-------------|------------|
| Firehose prefix is entirely dynamic expressions | Resolved path is just the bucket root, doesn't match your table's more-specific LOCATION | See "Known Limitations" below |
...
| Different partition column name | Your table uses `datehour` instead of `log_time` | Currently unsupported — the agent's queries use `log_time` in WHERE clauses |
...
- These tables are **permanent** and reused across sessions — no recreation overhead
