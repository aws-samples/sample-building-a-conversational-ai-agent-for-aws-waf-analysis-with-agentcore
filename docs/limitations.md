...
**You can stop a runtime session but you cannot list them.** `StopRuntimeSession` takes a `runtimeSessionId` and an `agentRuntimeArn`, and no operation in either AgentCore API returns the ids of a runtime's live sessions. `ListSessions` exists but is memory-scoped: its required inputs are `memoryId` and `actorId`. Verified 2026-09-12 against the service models the AWS CLI ships at `awscli/botocore/data/bedrock-agentcore*/`, 147 control-plane and 65 data-plane operations. So a stuck session has to be waited out rather than found and stopped.
