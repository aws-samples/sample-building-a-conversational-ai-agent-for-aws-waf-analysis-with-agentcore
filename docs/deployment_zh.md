...
2. **Docker Desktop**（含 buildx，用于构建容器镜像）。[安装 Docker Desktop](https://docs.docker.com/get-docker/)。另有两条路：[finch](https://github.com/runfinch/finch)，见[附录](#替代方案使用-finch)；或者什么容器工具都不装，见[在 AWS 上构建](#在-aws-上构建不需要-docker)。
3. **Node.js 18+**（构建前端）
