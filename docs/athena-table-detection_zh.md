...
2. **搜索 Glue Data Catalog** — 查找与 S3 路径匹配、包含 WAF 日志列（`action`、`httprequest`）**且按 `log_time` 分区**（Agent 分区裁剪所需）的现有表
3. **如果找到** — 验证表的分区格式和间隔是否与 S3 实际目录结构一致，一致则复用
4. **如果未找到** — 在 `waf_analysis_tmp` 数据库中自动创建表

### 自动自愈

当 Agent 自己的临时表（`waf_analysis_tmp.waf_logs_{webacl}`）的 location 失效时会自动自愈 — 例如你把某个 WebACL 的日志投递方式从 **Vended Logs**（`AWSLogs/.../WAFLogs/{scope}/{webacl}/`）切换到 **Firehose**（自定义的桶根前缀）。旧表仍指向已变空的原路径，导致查询返回 0 行，而 CloudWatch 指标仍显示有流量。

在下一次查询时，Agent 会将现有表的 `LOCATION` 与重新解析出的 S3 路径比较：

- **location 一致** → 原样复用（不重建、无停顿）
- **location 不同** → 删除并在正确路径重建临时表

这是自动发生的，无需手动 `DROP TABLE`。（删除外部表永远不会触碰底层 S3 数据。）

### 多 WebACL 共用桶

如果多个 WebACL 把日志投递到**同一个** Firehose 桶前缀，解析出的表 location 是共享的，否则会混入每个 WebACL 的记录。当 Agent 检测到表 location 并非当前 WebACL 专属时，会自动给每条日志查询加上 `webaclid` 过滤，使结果只反映正在调查的那个 WebACL。基于指标的数字本就按 CloudWatch 维度限定范围，不受影响。
...
- 表按 `log_time` 分区（partition projection）
- 分区间隔与 S3 实际目录结构一致
...
| 场景 | 失败原因 | 解决方法 |
|------|---------|---------|
| Firehose 前缀完全由动态表达式组成 | 解析出的路径仅为桶根目录，无法匹配更具体的用户表 LOCATION | Agent 自己的临时表会自愈（在解析路径上删除+重建）；但位于更深路径的*用户自建*表仍无法匹配 |
...
| 不同的分区列名 | 你的表使用 `datehour` 而非 `log_time` | Agent 会跳过它，并在旁边创建自己的 `log_time` 分区表（两张表指向相同 S3 数据） |
...
**重要：** 如果你的 Firehose 使用小时级分区（默认设置），Agent 会阻止日志明细查询，因为在生产流量下查询会超时。遇到这种情况时，Agent 会从知识库检索修复方法，并向你内联解释原因和一次性的 Firehose 配置更改。相同的步骤也可参见 [Firehose 优化指南](firehose-minute-partitioning_zh.md)。
...
- 这些表是**永久的**，跨会话复用 — 无重复创建开销（仅当其 location 失效时才会重建，参见上方"自动自愈"）
