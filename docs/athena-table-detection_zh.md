...
**注意：** 如果你的 Firehose 用的是小时级分区（默认就是），日志明细查询照样能跑，查询输出会提醒你一次扫描要读整整一个小时，成本更高。换成分钟级前缀是一次性的可选改动，能降低每次查询的扫描成本；Agent 会从知识库取出步骤，当场向你解释。见 [Firehose 优化指南](firehose-minute-partitioning_zh.md)。
...
2. **按天或更粗的分区，明细查询会被拒。** 检测这一步接受它们，查询再按扫描成本拦下。小时级和分钟级今天都能跑明细查询；小时级会在查询输出里提醒你，一次扫描要读整整一个小时。
3. **换过格式的桶，切换之前的历史默认读不到。** Agent 读最近的分钟级那段，并告诉你它从哪天开始。想读更早的小时级那段，让它按小时读整条时间线，它会替你在同一个桶上建那张表，代价是所有窗口都变成小时粒度。默认仍然是分钟级。
