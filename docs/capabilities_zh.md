...
Agent 一次调用跑完整个注入调查（SQLi / XSS / LFI）：整套流程在代码里执行，源 IP 画像那一步不会被跳过，返回的是分类和处置建议，而不是让你自己解读的原始表格。可以点名某条规则，也可以让它把所有注入规则过一遍。

> "JA4 t13d1516h2_8daaf6152771_b186095e22b6 是什么客户端？"

Agent 解码 JA4 TLS 指纹的结构——协议、TLS 版本、有无 SNI、密码套件和扩展数量——用来区分浏览器和自动化工具。它不识别具体是哪个应用。
...
Agent 支持 37 种预定义日志查询模板，覆盖 IP、规则、URI、标签、国家、Host 等维度。

> "今天，被 Block 的请求按国家、按小时分别是多少？"

> "过去 2 小时，访问 /admin 的请求里有多大比例命中了 SQLi 规则？"

> "今天，把被 Block 的请求按 JA4 指纹分组"

模板覆盖不到的问题，Agent 会自己拼一个聚合查询：过滤维度（动作、规则、IP、国家、方法、规则类型、标签、Host、JA4 指纹）任选，分组维度（客户端 IP、URI、国家、方法、动作、规则、规则类型、Host、User-Agent、Referer、标签、JA4 指纹，或时间桶）任选，结果可以是计数、命中率（满足某条件的占总数多少），或百分位（每个时间桶请求量的 p95/p99）。所以"哪条规则每小时触发最多""这个路径的拦截率"这类问题不需要专门的模板也能答。

## 更早的日志历史（两种格式并存的桶）

> "我要查切换到分钟级分区之前的日志"

桶里如果同时有小时级目录（切换之前）和分钟级目录（切换之后），默认读最近那段分钟级、精度完整，并告诉你更早的历史从哪天起。你要读，Agent 就按小时建一张覆盖整条时间线的表，切换之前的历史也能查了，代价是所有窗口都变粗到小时粒度。随时可以换回分钟级。你不用写任何 DDL，表由 Agent 建、也由它重建。
