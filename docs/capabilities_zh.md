...
Agent 定位拦截规则、计算该 IP 的 Allow Ratio，给出置信度评估。并展示实际命中的内容——规则的 match detail（SQLi/XSS）**以及**被检查的请求组件（query string、URI 或 cookie）——让你看清请求*为什么*被标记。密钥值（cookie、auth/会话令牌）会被掩码。
