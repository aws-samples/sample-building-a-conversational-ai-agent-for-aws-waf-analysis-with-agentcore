...
Agent 扫描 ALLOW 流量中的异常：高频 IP、异常 URI 多样性、自动化 User-Agent、数据中心 IP 无 bot 标签。

> "帮我评估 COUNT 规则，能不能转 Block？"

Agent 盘点所有 COUNT 规则，按风险分级，找到流量高峰时段，分析客户端行为，判断是否可以安全切换。
...
Agent 定位拦截规则、提取匹配详情、计算该 IP 的 Allow Ratio，给出置信度评估。
...
全维度画像：频率、URI 多样性、JA4 指纹、Bot 标签、Action 分布、NAT 检测。
...
确定性 HTML 巡检报告：规则指标、异常标记、DDoS 事件检测、Challenge 通过率。
...
- JA4 指纹查询使用开源 FoxIO 数据库，覆盖率有限 — 大多数指纹会显示为 "unknown"
