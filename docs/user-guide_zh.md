...
> **注意：** ROI 报告仅使用 CloudWatch Metrics + CloudWatch Logs Insights（不使用 Athena）。如果 WAF 日志通过 Firehose 发送到 S3，报告仍然可以生成——图表和数字来自 metrics，bot/DDoS 细节来自 CWL。如果未配置 CWL，bot 分类详情会省略，但报告仍会生成。
