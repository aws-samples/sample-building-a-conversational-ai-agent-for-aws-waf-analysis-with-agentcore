...
> **注意：** ROI 报告仅使用 CloudWatch Metrics + CloudWatch Logs Insights（不使用 Athena）。管理层报告只需要聚合指标，不需要 IP/URI 级别细节。如果 WAF 日志通过 Firehose 发送到 S3，报告仍然可以生成（图表来自 metrics，bot/DDoS 分类来自 CWL）。如果未配置 CWL，bot 分类详情会省略，但报告仍会生成。如需详细的攻击源分析，请使用安全巡检或直接对话查询。
