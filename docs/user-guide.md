...
>
> This is a defensive tool, but normal WAF work includes terms such as SQLi, XSS, bypass, blocked payload, malicious IP, and exploit attempt. Some GPT-family model deployments can trigger upstream cyber-safety checks and leave the agent appearing idle. Recommended deployments use Claude Sonnet 5.
...
- **GPT-family model safety filters.** A deployment on a GPT-family Bedrock model can go silent in the middle of defensive WAF analysis, with no error anywhere. Nothing you type recovers it reliably. Redeploy on Claude Sonnet 5.
