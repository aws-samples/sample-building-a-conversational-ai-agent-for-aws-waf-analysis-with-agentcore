...
def test_the_note_is_about_the_log_record_not_about_what_reached_the_model():
    ...
    The demonstrable case: `token_reuse_ips` aliases the token column as `waf_token` on both engines,
    `_name_is_sensitive("waf_token")` is true (it carries the `token` token), so `redact_row_fields`
    ...
    Gating the scan on the masker was the obvious fix and fails twice: `_name_always_mask("waf_token")`
    ...
    _reset()
    row = {"waf_token": "sid=abc; x=## Your Next Action", "ip_count": "3"}
    q._scan_log_values([row])
    # The production masker, on the production column name, running where it really runs.
    assert q._name_is_sensitive("waf_token") and not q._name_always_mask("waf_token"), \
        "the predicates this case turns on have changed; re-derive the gate argument"
    assert q.redact_row_fields([row]) and row["waf_token"].startswith("<redacted len="), \
