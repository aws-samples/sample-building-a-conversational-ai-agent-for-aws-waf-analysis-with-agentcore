...
def _build_agent_hourly_table(s3_path: str, region: str, webacl_name: str, layout: dict) -> str:
    ...
    if not _validate_waf_log(s3_path):
        raise RuntimeError(
            ...
            f"JSON (.gz) logs only; if these are Parquet logs, create an Athena table whose "
            f"LOCATION is this same path, and the agent will find and query it automatically.")
