...
def list_webacls(scope: str = "CLOUDFRONT", region: str = "us-east-1") -> str:
    ...
    Args:
        scope: AWS WAF scope — "CLOUDFRONT" (global) or "REGIONAL". Default CLOUDFRONT.
...
def get_waf_config(webacl_name: str, scope: str = "CLOUDFRONT", region: str = "us-east-1") -> str:
    ...
    if caps["bot_control"] != "none":
        lines.append("- Is the site SPA? Is AWS WAF Client SDK integrated? (affects Bot Control recommendations)")
...
def _normalize_redacted(entries: list) -> tuple:
    ...
    Unknown keys are skipped rather than guessed at. If AWS ever accepts a fifth type, a query
    filtering on it would go unwarned, which is why `tests/` pins the accepted set against this
