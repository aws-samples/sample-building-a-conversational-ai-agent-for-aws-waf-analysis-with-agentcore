...
def run_logs_query(
    ...
    if log_group:
        ...
        # **This path never reaches `query_logs`, so it has to record for itself.** It names the log
        # group rather than the session WebACL, which was not consulted: the caller passed a destination
        # and the WebACL in state has nothing to do with the answer.
