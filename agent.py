...
- **Athena partition-path timezone.** For S3/Athena logging specifically, another cause of "0 log rows but metrics show traffic" is that the S3 partition directories (e.g. the dt/log_time folders) are written in LOCAL time while the agent prunes them as UTC. Vended logs and Firehose (via CustomTimeZone) are handled automatically; a custom/bring-your-own table partitioned in local time is not. When you see this symptom on an Athena backend, ask whether the log partitions are in local time; if so, call set_log_table with partition_timezone set to their IANA zone (e.g. 'America/New_York') and retry. Do NOT keep re-running the same query expecting a different result.
